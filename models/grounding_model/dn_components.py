...
def prepare_for_dn(dn_args, embedweight, batch_size, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    for i in range(len(targets)):
        diff = targets[i]['actioness'][1:] - targets[i]['actioness'][:-1]
        start_idx = (diff == 1).nonzero(as_tuple=True)[0].item() + 1
        end_idx = (diff == -1).nonzero(as_tuple=True)[0].item()
        targets[i]['labels'] = targets[i]['actioness'][start_idx:end_idx+1]
        targets[i]['boxes'] = targets[i]['boxs'].bbox
