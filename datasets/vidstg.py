...
class VidSTGDataset(data.Dataset):
    ...
    def __init__(self, cfg, split, transforms=None) -> None: 
        ...
        self.frame_dir = os.path.join(self.data_dir,'frame') # only used if frames available
    ...
    def make_data_pairs(self,anno_file):
        ...
        #vstg_anno_path = os.path.join(vstg_anno_dir, self.split + '_sample.json')
        print(f"Load Anno Json from outside all general {vstg_anno_path}")
